	...
	data: function(){
  	...
  	template: 
  		'<div id="wydiv">'+
			...
			'<div id="WY-lm-panel">'+
				'<div class="WY-lm-container" id="remote-container"><div class="SPEAK_UI" :class="{SPEAK_UI_pro:speak}"><div class="animation_rel"><div class="SPEAK_animation" :style="{height:randomHeight}"></div></div></div></div>'+
				'<div class="WY-lm-container"><div class="SPEAK_UI" :class="{SPEAK_UI_pro:speak}"><div class="animation_rel"><div class="SPEAK_animation" :style="{height:randomHeight}"></div></div></div></div>'+
				'<div class="WY-lm-container"><div class="SPEAK_UI" :class="{SPEAK_UI_pro:speak}"><div class="animation_rel"><div class="SPEAK_animation" :style="{height:randomHeight}"></div></div></div></div>'+
				'<div class="WY-lm-container"><div class="SPEAK_UI" :class="{SPEAK_UI_pro:speak}"><div class="animation_rel"><div class="SPEAK_animation" :style="{height:randomHeight}"></div></div></div></div>'+
			'</div>'+
...
//				this.randomHeight=Math.ceil(12*random)+'px'
//			}.bind(this),300)
		},
		getWYtoken: function(){
...
//				debug: true
		...
		joinChannel: function(){
			...
			if(this.isTeacher){
				sessionConfig.splitMode = WebRTC.LAYOUT_SPLITCUSTOM; //主播和连麦者的布局模式：自定义布局
			...
			this.netcall.startDevice({
		      ...
		      height: 480
				...
				this.netcall.setVideoViewSize(
					{
						width:(this.isTeacher ? 275 : 137), 
						height:(this.isTeacher ? 200 : 100), 
						cut:false
					}
				);
				//切换为连麦角色
				...
				this.$emit('start-role-player','test');
		   		console.log('<网易直播组件>',this.liveMsg);
			}.bind(this)).catch(function(error) {
